# -*- coding: utf-8 -*-
"""
Created on Sun Dec  8 21:13:28 2019
Nathan Marshall
...
nx = 20 #number of grid positions in x
ny = 20 #number of grid positions in y
...
psi[0] = 0 #boundary conditions for inflow
psi[ny-1] = 0.0001 #boundary conditions for inflow
...
def SOR(psi, w, numiter):
    ...
    for i in range(0, nx):   
        psi[i][0] = (4*psi[i][1] - psi[i][2] + 2*h)/3
                
def vort_wall(psi, w):
    '''Determine the vorticity boundary values from the stream function psi.'''
    for i in range(0, ny):  
         w[i][0] = 2*(psi[i][0] - psi[i][1])/h**2 - 2/h#left side
    w[:,ny-1] = -2 * psi[:,ny-2]/h**2 #right side
    w[0] = -2 * psi[1]/h**2 #top side
    w[ny-1] = -2 * psi[ny-2]/h**2 #bottom side
